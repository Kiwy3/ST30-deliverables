\begin{tikzpicture}[]
    ...
    % lvl 2
    \node (autodnn)[field, right of = opt_llm, xshift = 1.5cm, yshift = 1cm, fill=teal!30]{Auto-DNN};
    ...
    % lvl 3
    \node (hpo)[field, right of = autodnn, xshift = 1.3cm,yshift = -1cm, fill = cyan!50]{HPO};
    \node (nas)[field, right of = autodnn, xshift = 1.3cm,yshift = 1cm, fill = green!30]{NAS};
    ...
    % lvl4 nas
    \node (nas_scratch)[art, right of = nas, xshift = 1.5cm,yshift = -0.1cm, fill = green!20]{
        \textbf{From Scratch}
    };
    \node (nas_pruning)[art,above right  of = nas, yshift = 0.1cm,xshift = 1.7cm, fill = green!20]{
        \textbf{Pruning}
    };
    ...
    \node (current)[draw,thick, dashed, rounded corners, 
        fit = (hpo)(hpo_ft), inner sep = 0.1cm,
        label = 35 :{Travail actuel}]{};

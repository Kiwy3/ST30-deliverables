...
To sum up equation \ref{eq : autodnn_form}, we want to optimize $a$, so that $f(a)$ is the best when $a$ is used to train the model (optimize the weights). The \acrshort{autodnn} problem in characterized, is the worst case, by theses properties : 
\begin{itemize}
    \item \textbf{\acrfull{mvop}} : Variables can be continuous (e.g. learning rate), discrete ordinal (e.g number of layers or neurons) or discrete categorical (e.g. type of activation function). The search space of the problem contains conditionnality, i.e. the size of a variables may depend on other variable (e.g. the number of neurons depend on the number of layers). These properties requires specific optimization methods, or conversion of variables. %#TODO - cite talbi
...
\section{Optimization Algorithms taxonomy}
\label{sec : opt_algo}
...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.6\linewidth]{assets/img/chap_2/plots/fda.png}
    \caption{Example of a Partition Based Optimization on 2D himmelblau}
    \label{fig:pbo}
\end{figure}
%#TODO - rework the figure 
